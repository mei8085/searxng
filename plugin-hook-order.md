...
1. **`PluginStorage.plugin_list` 是 `set[Plugin]` 类型**（见 `searx/plugins/_core.py:195`）

2. **插件对象的哈希值就是其内存地址**（见 `searx/plugins/_core.py:115-121`）

```python
# searx/plugins/_core.py:195
plugin_list: set[Plugin]

# searx/plugins/_core.py:115-121
def __hash__(self) -> int:
    return id(self)  # 直接返回对象内存地址作为哈希值
```

### 为什么顺序必然不可依赖

这两个事实结合起来，从机制上决定了插件遍历顺序**在工程上完全不可依赖**：

1. **Python `set` 的遍历顺序由元素的哈希值决定**，哈希值决定了元素在哈希表中的存储位置，遍历顺序就是哈希表的存储顺序
2. **插件的哈希值 = 对象内存地址**，每次进程启动时，插件对象被分配的内存地址都是不同的
3. **因此，每次进程启动，插件在 set 中的存储位置都会变化，遍历顺序自然不同**

这不是"可能变化"，而是**必然变化**。即使完全相同的配置、相同的机器、相同的 Python 版本、关闭了哈希随机化，只要进程重启，内存地址就会变，遍历顺序就会变。

> 哈希随机化不是主要原因，也不需要引用它。上述两点机制已经足够得出"顺序不可依赖"的结论。

### 对业务逻辑的影响

- **`pre_search` 短路优先级完全不可控**：如果多个插件都可能返回 `False`，无法预知哪个会先触发
- **`on_result` 过滤优先级完全不可控**：如果多个插件都可能过滤同一结果，无法预知哪个会先生效
- **`post_search` 结果追加顺序完全不可控**：多个插件返回的结果在最终列表中的顺序无法预知
...
2. **顺序不可控的机制原因**：插件哈希值 = 内存地址，每次进程重启地址必然变化，set 遍历顺序由哈希值决定
